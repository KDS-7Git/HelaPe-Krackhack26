...
contract PayStreamEnhanced is Ownable, ReentrancyGuard {
    struct Stream {
        address sender;
        address recipient;
        uint256 deposit;
    ...
    function getTotalAvailable(uint256 streamId) public view returns (uint256) {
        ...
        uint256 pendingYield = (balance * YIELD_RATE * timeElapsed) / (SECONDS_PER_YEAR * BASIS_POINTS);
    ...
    function cancelStream(uint256 streamId) external nonReentrant {
        ...
        uint256 vested = getVestedAmount(streamId);
        uint256 totalBalance = stream.deposit + stream.yieldAccrued;
        ...
        uint256 unvested = totalBalance + totalBonuses - vested;
        ...
        // Settle vested amount with recipient
